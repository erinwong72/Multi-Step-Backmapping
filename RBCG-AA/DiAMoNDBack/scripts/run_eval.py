...
def load_trainer(system_name, model_name,  last_milestone=None, 
    ...
    model_path = Path(f"/home/erin/Documents/DiAMoNDBack/trained_models/{train_name}/{MODEL_INFO}")
...
def iterative_inference(data, system_name, cond_type, n_cond_res, model_name, pdb_save_names=[], dtime=50,
    ...
    if len(n_sample_list) > 1:
        sample_rmsd = crd_to_mdtrj(all_seqs, all_crds_ref, all_crds, end_idx_list,
                    pdb_save_names, n_sample_list=n_sample_list, save_dir=save_dir)
        # save sample rmsd
        with open(f'sample_rmsd_{pdb_save_names[0]}.txt', 'w') as f:
            f.write('\n'.join([str(i) for i in sample_rmsd]))
    else:
        crd_to_mdtrj(all_seqs, all_crds_ref, all_crds, end_idx_list,
                    pdb_save_names, n_sample_list=None, save_dir=save_dir)
