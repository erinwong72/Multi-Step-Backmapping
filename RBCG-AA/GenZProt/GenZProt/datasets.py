...
PROTEINFILES = {}
PED_PDBs = glob.glob('/gpfs/home/eswong/GenZProt/data/genzprot_ped/*.pdb')
...
# eIF4E_PDBs = glob.glob('/gpfs/home/eswong/GenZProt/data/eIF4E/*.pdb')
# for PDBfile in eIF4E_PDBs:
#     ID = PDBfile.split('/')[-1].split('.')[0][5:]
#     dct = {ID: {'pdb_path': PDBfile,
#             'traj_paths': PDBfile,
#             'file_type': 'pdb'
#                     }
#                     }
#     PROTEINFILES.update(dct)
sarscov2_trajs = glob.glob('/gpfs/home/eswong/GenZProt/data/sarscov2/*.xtc')
for PDBfile in sarscov2_trajs:
    ID = PDBfile.split('/')[-1].split('.')[0]
    dct = {ID: {'pdb_path': '/gpfs/home/eswong/GenZProt/data/sarscov2/sarscov2_top_noH_reg.pdb',
            'traj_paths': PDBfile,
            'file_type': 'xtc'
                    }
                    }
...
def traj_to_info(traj):
    ...
    n_cg = len(reslist)
    atomic_nums, protein_index = get_atomNum(traj)
    # print(list(table.loc[table.newSeq==reslist[2]].name))
    ...
    for i in range(len(resn)):
        # for j in range(len(atomn[i])):
        #     print(atomn[i])
        #     print(resn[i], atomn[i][j])
        #     print(np.where(np.array(core_atoms[resn[i]])==atomn[i][j])[0][0])
