...
def shuffle_traj(traj):
    ...
    full_idx = shuffle(full_idx, random_state=123)
...
def loop(loader, optimizer, device, model, beta, gamma, delta, eta, zeta, epoch, 
    ...
    for i, batch in enumerate(loader):
        ...
        print("prot indices: ", batch['prot_idx'])
        ...
        ## stuff added here
        if torch.isnan(loss_graph).any():
            print("NaN detected in graph loss")
        if torch.isnan(loss_inter).any():
            print("NaN detected in interaction loss")
